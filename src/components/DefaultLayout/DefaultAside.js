...
class DefaultAside extends Component {
    ...
    render() {
        ...
        const { products, children, ...attributes } = this.props;
                ...
                <TabContent activeTab={this.state.activeTab}>
                    <TabPane tabId="1">
                        <ListGroup className="list-group-accent" tag={'div'}>
                            ...
                            {logs &&
                                logs.map((log, index) => {
                                    const product = products.find(prod => prod._id === log.product);
                                    const prefix = log.stockChange > 0 ? 'Added' : 'Consumed';
                                    return (
                                        <ListGroupItem
                                            key={index}
                                            className={`list-group-item-accent-${
                                                log.stockChange > 0 ? 'primary' : 'danger'
